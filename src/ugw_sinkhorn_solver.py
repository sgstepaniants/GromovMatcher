...
class UGWSinkhornSolver(object):
    ...
    def init_plan(a, b, init):
        if init is not None:
            ...
            return a[:, None] * b[None, :] / np.sqrt(a.sum() * b.sum())
    ...
    def compute_local_cost(self, pi, a, Cx, b, Cy, rho, eps, lmbda=0, otcost=None, quadratic=True, weights=None):
        ...
        if otcost is not None:
            if quadratic:
                E = (1 - lmbda) * E + lmbda * torch.sum(otcost * pi) * otcost
            else:
                E = (1 - lmbda) * E + lmbda/2 * otcost
        if weights is not None:
            E = E * weights
        return E + rho * kl_mu + rho * kl_nu + eps * kl_pi
    ...
    def ugw_cost(self, pi, gamma, a, Cx, b, Cy, rho, eps):
    ...
    def sinkhorn_procedure(self, T, u, v, a, b, rho, eps, exp_form=True):
        ...
        if exp_form:
            ...
            for j in range(self.nits_sinkhorn):
                ...
                v = s_x(u)
                u = s_y(v)
                if eps * (u.log() - u_prev.log()).abs().max().item() < self.tol_sinkhorn:
                    break
    ...
    def ugw_sinkhorn(self, a, Cx, b, Cy, rho, eps, init=None, lmbda=0, otcost=None, quadratic=True, weights=None):
        ...
        while i < self.nits and time.time() < endtime:
            pi_prev = pi.clone()
            Tp = self.compute_local_cost(pi=pi, a=a, Cx=Cx, b=b, Cy=Cy, rho=rho, eps=eps, \
                                         lmbda=lmbda, otcost=otcost, quadratic=quadratic, weights=weights)

            mp = pi.sum()
            ug, vg, gamma = self.sinkhorn_procedure(Tp, ug, vg, a, b, mp * rho, mp * eps)
            gamma = (mp / gamma.sum()).sqrt() * gamma

            Tg = self.compute_local_cost(pi=gamma, a=a, Cx=Cx, b=b, Cy=Cy, rho=rho, eps=eps, \
                                         lmbda=lmbda, otcost=otcost, quadratic=quadratic, weights=weights)
